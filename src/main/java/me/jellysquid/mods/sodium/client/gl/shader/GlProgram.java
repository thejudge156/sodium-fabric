...
public class GlProgram<T> extends GlObject implements ShaderBindingContext {
    ...
    public void bind() {
        GLES32.glUseProgram(this.handle());
    }
    ...
    public void delete() {
        GLES32.glDeleteProgram(this.handle());
    ...
    public <U extends GlUniform<?>> U bindUniform(String name, IntFunction<U> factory) {
        int index = GLES32.glGetUniformLocation(this.handle(), name);
    ...
    public GlUniformBlock bindUniformBlock(String name, int bindingPoint) {
        int index = GLES32.glGetUniformBlockIndex(this.handle(), name);

        if (index < 0) {
            throw new NullPointerException("No uniform block exists with name: " + name);
        }

        GLES32.glUniformBlockBinding(this.handle(), index, bindingPoint);
    ...
    public static class Builder {
        ...
        public Builder(Identifier name) {
            this.name = name;
            this.program = GLES32.glCreateProgram();
        }

        public Builder attachShader(GlShader shader) {
            GLES32.glAttachShader(this.program, shader.handle());
        ...
        public <U> GlProgram<U> link(Function<ShaderBindingContext, U> factory) {
            GLES32.glLinkProgram(this.program);

            String log = GLES32.glGetProgramInfoLog(this.program);

            if (!log.isEmpty()) {
                LOGGER.warn("Program link log for " + this.name + ": " + log);
            }

            int result = GLES32.glGetProgrami(this.program, GLES32.GL_LINK_STATUS);
        ...
        public Builder bindAttribute(String name, int index) {
            GLES32.glBindAttribLocation(this.program, index, name);
